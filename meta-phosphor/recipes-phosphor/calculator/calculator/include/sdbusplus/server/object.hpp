...
//繼承構造函數 如果 using 聲明引用了正在定義的類的直接基類的構造函數（例如 using Base::Base;）
//則在初始化派生類時，該基類的所有構造函數（忽略成員訪問）對重載決議可見。
...
struct compose_impl : compose_inherit_t<T>, compose_impl<Rest...>
{
    ...
    { 
        printf("compose_impl(bus_t& bus, const char* path) :T(bus, path), compose_impl<Rest...>(bus, path)\n ");
    }
...
struct compose_impl<T> : compose_inherit_t<T>
{
    ...
    {
        printf(" compose_impl<T> : compose_inherit_t<T> \n");
